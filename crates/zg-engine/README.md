...
Each workspace has one root directory. File records store paths relative to that root; scanning a subdirectory does not change the path base. Each physical index generation owns `u32` file IDs in its file records. Writers build an in-memory path-to-ID cache from those records once, reuse IDs already known by scanning, and reserve new IDs without an extra database or allocation journal. Source write intent and recovery use the existing pending-file journal. Rebuilds may assign different IDs; deleted records and unwritten reservations have no persistent identity guarantee. Workspace renaming and relocation preserve the active records and their IDs. Directory membership keeps compact 4-byte `u32` IDs. Writers derive the index-local directory map from source membership once and allocate in memory. A rebuildable directory-map snapshot is checkpointed with the index so readers can resolve paths without scanning files; retrieval documents never duplicate ancestor path strings. Unicode query paths use UTF-8 components separated by `/`; identity keys and stored source paths retain a lossless native encoding, including non-Unicode names. This does not promise that native index files or arbitrary filenames can be moved between operating systems. Reads resolve absolute paths against the workspace's current location.
...
After moving a workspace, its next index operation updates the registered root when the original workspace no longer exists. Copying a live workspace requires a different name for the copy. File identity is scoped to each workspace, so a copy may retain its indexed file records while using its own workspace name.
...
- `ZvecGrep::drop_index` removes the index, manifest, legacy identity artifacts, and workspace name reservation while preserving source files.
...
Entity metadata uses the domain types throughout extraction, storage, and API results. Each metadata category declares its indexed fields by JSON key and type; storage uses those declarations for schema creation, writes, and filters. Storage prepares the complete JSON payload and indexed values once per entity, then reuses the indexed values across retrieval records. Each entity stores its complete metadata once as JSON. Windows share their owner's metadata. Search fuses lightweight hits first, then batch-loads the selected entities and matching fragments.

Symbol filters use `alias`, `class`, `enum`, `function`, `interface`, `module`, and `value`. Classes include structs, records, and Rust implementation blocks; functions include methods and constructors; interfaces include traits and protocols. Unclassified code retains metadata without a symbol category and remains searchable without a category filter. Code signatures preserve declaration modifiers and decorators without implementation bodies.

Index version 5 is the next format after released version 4. It uses index-local identities and numeric directory membership alongside this metadata layout. A version mismatch reports an error and requires an explicit rebuild (`IndexOptions::rebuild` or `zg index --rebuild`); indexing and query refresh never automatically upgrade the format. Old single-root manifests can still supply names, embedding, and discovery settings for the rebuild. Manifest version 4 retains the flat JSON representation, ignores a legacy workspace UUID `id`, and writes only the workspace name. Manifest migration does not change the physical index format.
...
Full-text search uses the Jieba dictionaries shipped and discovered by zvec-rust 0.7.1. Native distributions must include the SDK’s `data/jieba_dict` directory alongside its shared library. Storage schema version 10 removes the shared identity catalog and scopes both file and directory IDs to the physical index. Old catalogs are removed only after a successful rebuild publication or explicit drop. Files without a supported extractor are skipped; format recognition alone does not imply parsing support.
